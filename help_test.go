...
func TestLogHelpDescribesStepByStepWorkflow(t *testing.T) {
	...
	for _, needle := range []string{
		"proctor log - record a verification step with a screenshot",
		"--scenario ID",
		"--session SESSION",
		"--surface SURFACE",
		"--screenshot PATH",
		"--action TEXT",
		"chronological visual record",
...
func TestAnalyzeHelpDescribesVisionAnalysis(t *testing.T) {
	text, ok, err := commandHelp([]string{"analyze", "--help"})
	if err != nil {
		t.Fatal(err)
	}
	if !ok {
	...
	for _, needle := range []string{
		"proctor analyze - run AI vision analysis on screenshots",
		"--scenario ID",
		"ANTHROPIC_API_KEY",
		"ANTHROPIC_MODEL",
		"ANTHROPIC_BASE_URL",
		...
		"Compare what it sees against the scenario requirements",
	} {
		if !strings.Contains(text, needle) {
			t.Fatalf("expected analyze help to mention %q, got:\n%s", needle, text)
		}
	}
}
...
func TestHelpTopicSupportsLog(t *testing.T) {
	...
	if !strings.Contains(text, "proctor log - record a verification step with a screenshot") {
		t.Fatalf("expected log topic help, got:\n%s", text)
	}
}

func TestHelpTopicSupportsAnalyze(t *testing.T) {
	text, err := topicHelp([]string{"analyze"})
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(text, "proctor analyze - run AI vision analysis on screenshots") {
		t.Fatalf("expected analyze topic help, got:\n%s", text)
	}
}

func TestRootHelpMentionsLogAndAnalyze(t *testing.T) {
	text, ok, err := commandHelp([]string{"--help"})
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, needle := range []string{
		...
		"AI vision analysis",
		"record each step with a screenshot",
	} {
		if !strings.Contains(text, needle) {
			t.Fatalf("expected root help to mention %q, got:\n%s", needle, text)
		}
