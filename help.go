...
Minimum lengths: pre-note 20 chars, verifications/observations/comparisons 40 chars + 4 distinct words.
...
Mandatory next step for the agent after reading this help:
  ...
  6. re-read each recorded screenshot and call proctor verify with a
     verification stating whether the evidence satisfies the scenario contract
...
Typical web workflow:
  ...
  # Re-read each recorded screenshot and write a verification on whether it satisfies the contract:
  proctor verify \
    --scenario happy-path \
    --session auth-browser-1 \
...
After start:
  ...
  - re-read each recorded screenshot and close the loop with proctor verify --scenario X --session Y --verification "..."
...
Important:
  ...
  - every recorded screenshot evidence enters pending-verification and must
    be closed with proctor verify --scenario X --session Y --verification "..." (minimum 40 chars)
...
func verifyHelpText() string {
	return `proctor verify - write a verification on whether the evidence satisfies the contract

Usage:
  proctor verify \
    --scenario ID \
    --session SESSION \
    --verification "state whether the screenshot satisfies the contract and why"

Required:
  --scenario ID      Scenario id matching a recorded evidence entry
  --session SESSION  Session id used when the evidence was recorded
  --verification TEXT     Verification (minimum 40 chars, 4+ distinct words, must include a judgment word)
...
still pending-verification. Verifications are stored alongside the evidence for
the contract report. Each evidence record can only be verified once.

Good verifications connect what is visible to the contract:
...
Bad verifications describe pixels without judging the contract:
...
Example:
  proctor verify \
    --scenario happy-path \
    --session auth-browser-1 \
    --verification "This satisfies the contract because the dashboard greeting and Sign out link match the expected behavior."
`
}
...
proctor verify --scenario ID --session SESSION --verification "..." with a
verification stating whether the evidence satisfies the contract. proctor done
