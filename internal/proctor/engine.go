...
// LogStep records one step in the agent's verification walkthrough: what
// the agent did, along with a screenshot of the result. Steps build a
// chronological visual record that can later be analyzed with proctor analyze.
func LogStep(store *Store, run Run, opts LogStepOptions) (ScreenshotLogEntry, error) {
	...
	if len(action) < MinObservationNotesLength {
		return ScreenshotLogEntry{}, fmt.Errorf(
			"action description must be specific (got %d chars, minimum %d)",
...
// AnalyzeOptions contains the parameters for analyzing screenshots.
type AnalyzeOptions struct {
	ScenarioID string
	SessionID  string // optional: filter to one session
}

// AnalyzeScreenshots uses the Claude vision API to analyze all screenshots
// for a scenario, comparing what's visible against the scenario requirements.
// It reads screenshots from both the screenshot log and recorded evidence,
// sends each to the API, and stores the results in analysis.jsonl.
func AnalyzeScreenshots(store *Store, run Run, client *VisionClient, opts AnalyzeOptions) ([]VisionAnalysis, error) {
	scenarioID := strings.TrimSpace(opts.ScenarioID)
	if scenarioID == "" {
		return nil, fmt.Errorf("analyze: --scenario is required")
	}
	scenario, ok := findScenario(run, scenarioID)
	if !ok {
		return nil, fmt.Errorf("unknown scenario: %s", scenarioID)
	}

	// Gather pre-notes for context.
	preNotes, err := store.LoadPreNotes(run)
	if err != nil {
		return nil, fmt.Errorf("load pre-notes: %w", err)
	}
	var preNoteTexts []string
	for _, note := range preNotes {
		if note.Scenario == scenarioID {
			preNoteTexts = append(preNoteTexts, note.Notes)
		}
	}
	preNoteContext := strings.Join(preNoteTexts, "; ")

	// Collect screenshot targets from the screenshot log.
	var targets []screenshotTarget
	logEntries, err := store.ScreenshotLogLedger(run).LoadForScenario(scenarioID)
	if err != nil {
		return nil, fmt.Errorf("load screenshot log: %w", err)
	}
	for _, entry := range logEntries {
		if opts.SessionID != "" && entry.SessionID != opts.SessionID {
			continue
		}
		absPath := filepath.Join(store.RunDir(run), entry.ScreenshotPath)
		targets = append(targets, screenshotTarget{
			Path:       absPath,
			LogEntryID: entry.ID,
			Action:     entry.Action,
		})
	...
	// Collect screenshot targets from recorded evidence.
	evidence, err := store.LoadEvidence(run)
	if err != nil {
		return nil, fmt.Errorf("load evidence: %w", err)
	}
	for _, ev := range evidence {
		if ev.ScenarioID != scenarioID {
			continue
		}
		if opts.SessionID != "" && ev.Provenance.SessionID != opts.SessionID {
			continue
		}
		for _, art := range ev.Artifacts {
			...
			absPath := filepath.Join(store.RunDir(run), art.Path)
			targets = append(targets, screenshotTarget{
				Path:       absPath,
				EvidenceID: ev.ID,
			})
	...
	if len(targets) == 0 {
		return nil, fmt.Errorf("no screenshots found for scenario %s", scenarioID)
	}

	analysisLedger := store.AnalysisLedger(run)
	var results []VisionAnalysis
	...
	for _, target := range targets {
		...
		if err != nil {
			return results, fmt.Errorf("analyze %s: %w", filepath.Base(target.Path), err)
		}

		analysis.ID = newID("va")
		analysis.RunID = run.ID
		analysis.ScenarioID = scenarioID
		analysis.EvidenceID = target.EvidenceID
		analysis.LogEntryID = target.LogEntryID
		analysis.ScreenshotPath = target.Path

		if err := analysisLedger.Append(*analysis); err != nil {
			return results, fmt.Errorf("store analysis: %w", err)
		}
